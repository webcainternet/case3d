...
$result = mysql_query("select product_id from case3d_product order by product_id desc limit 1");
...
if (isset($novoid)) {
	...
	$iquery1 = "INSERT INTO  `case3d`.`case3d_product` (
	`product_id` ,
	`model` ,
	`sku` ,
	`upc` ,
	...
	if (!$rquery1) {
		...
		$iquery2 = "INSERT INTO  `case3d`.`case3d_product_description` (
			`product_id` ,
			`language_id` ,
			`name` ,
			`description` ,
		...
		if (!$rquery2) {
			...
			$iquery3 = "INSERT INTO  `case3d`.`case3d_product_to_layout` (
				`product_id` ,
				`store_id` ,
				`layout_id`
				)
			...
			if (!$rquery3) {
				...
				$iquery4 = "INSERT INTO  `case3d`.`case3d_product_to_store` (
					`product_id` ,
					`store_id`
					)
					VALUES (
